...
class FirstPage extends React.Component {
    ...
    render() {
        ...
        return <>
            ...
            {goods.map((item) => {
                return <div key={item.id}  className="jest">
                    <div className="good_in_cart">
                        <h3 className="nm">Name:{item.name}</h3>
                        <h4 className="pr">Price:{(item.price * (100 - item.discount)) / 100}$</h4>
                        {/* <input type="number" name="numberOfGoods" className="numberOfGoods" defaultValue="0" onChange={(e) => this.inputOnChange(e, item)} />*/}
                        <h3 className="nm">Are you sure about this item?</h3>
                    </div>
                    <div className="mm">
                        <input className="check" type="checkbox" name="inCart" checked onChange={(e) => this.checkboxOnChange(e, item)} />
                        <button className="puk" onClick={(e) => this.props.deleteFromCart(item)}>Delete</button>
                    </div>
            ...
            <Link to="/secondPage"><button className="puk">Next page</button></Link>
        </>
    }
}
